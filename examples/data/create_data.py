...
line_1 = convert(args.start_value[0]) * random.uniform(.5, 1.5)
line_2 = convert(args.start_value[0]) * random.uniform(.5, 1.5)
line_3 = convert(args.start_value[0]) * random.uniform(.5, 1.5)
...
for i in xrange(length):
    out.append({'date': current_date, 'line_1': line_1, 'line_2': line_2, 'line_3': line_3})
    nb = convert(magnitude * (random.random()-.5))
    line_1 += nb
    line_2 += nb
    line_3 += nb
	# if args.percentage:
